...
const buildStatusMessage = ({ actor, oldStatus, newStatus, note, extraContext }) => {
    ...
    if (newStatus === 'completed' && oldStatus !== 'completed') {
        return `Task completed by ${actor.name}${roleLabel}`;
    }
...
const createHistoryAndComment = async ({
    ...
    let comment = null;
    if (createComment) {
        const content = (commentContent || message || '').toString().trim();
        if (!content) {
            createComment = false;
        }
    }
    ...
    if (createComment) {
        ...
        comment = await Comment.findOne(commentQuery).lean();
        if (!comment) {
    ...
    const addToSetPayload = { history: history._id };
    if (comment && comment._id) {
        addToSetPayload.comments = comment._id;
    }
...
const recordTaskUpdate = async ({
    ...
    const createComment = Boolean(safeNote);
    const commentContent = safeNote;
